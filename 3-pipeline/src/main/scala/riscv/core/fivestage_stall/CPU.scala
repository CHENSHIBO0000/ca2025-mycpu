...
// Five-Stage Pipelined CPU with Stalling
//
// Pipeline stages: IF → ID → EX → MEM → WB
//
// Architecture:
// - Classic five-stage RISC pipeline with full separation of execution phases
// - Each stage has dedicated pipeline register (IF2ID, ID2EX, EX2MEM, MEM2WB)
// - Conservative hazard resolution through pipeline stalls (bubbles)
//
// Hazard handling:
// - Data hazards: Stall pipeline when ANY RAW dependency detected
//   * EX-stage dependency: Stall 1 cycle for ALU results
//   * MEM-stage dependency: Stall 2 cycles for load results
// - Control hazards: Flush IF and ID stages on taken branches (resolved in EX)
// - Load-use hazards: Implicitly handled by RAW stall logic
//
// Performance characteristics:
// - CPI: ~1.5-2.0 (frequent stalls on dependencies)
// - Complexity: Medium (stall logic simpler than forwarding)
// - Educational value: Demonstrates conservative pipeline design
//
// Stalling strategy:
// - Insert bubbles (NOP) into pipeline when hazard detected
// - Freeze PC and IF/ID stages until dependency resolved
// - Simple but inefficient compared to forwarding approach

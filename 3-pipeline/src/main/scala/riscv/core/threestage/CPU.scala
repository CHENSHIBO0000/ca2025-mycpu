...
//
// Pipeline stages: IF → ID → EX/MEM/WB (folded)
//
// Architecture:
// - Minimal 3-stage pipeline with IF and ID stages separate
// - EX, MEM, and WB folded into single combinational stage
// - Simplest pipeline introducing basic control-flow redirection
//
// Hazard handling:
// - Control hazards: Flush IF and ID stages on taken branches/jumps
// - Data hazards: Implicit resolution through combinational execute stage
// - Load-use hazards: No stalling needed (execute completes combinationally)
//
// Performance characteristics:
// - CPI: ~1.2-1.4 (branch penalties only)
// - Complexity: Low (minimal pipeline registers)
// - Educational value: Introduces pipelining concepts with minimal complexity
//
// CLINT integration:
// - Interrupt detection in ID stage
// - Trap entry redirects IF immediately
// - CSR updates atomic with instruction commit
